...
export default function TareasProgreso() {
    ...
    // -------------------- DATOS DE SESIÓN/PERSISTENCIA (SessionStorage) --------------------
    // Datos del Departamento
    const depId = getSessionItem("last_depId");
    const depNombre = getSessionItem("last_depNombre") || "Departamento";
    const depSlug = getSessionItem("last_depSlug");

    // Datos del Proyecto
    const [idProyecto, setIdProyecto] = useState(
        location.state?.idProyecto || 
        getSessionItem("last_idProyecto")
    );
    const [nombreProyecto, setNombreProyecto] = useState(
        location.state?.nombreProyecto || 
        getSessionItem("last_nombreProyecto") || 
        proyectoSlug.replace(/-/g, " ")
    );
    const [descripcionProyecto, setDescripcionProyecto] = useState(
        location.state?.descripcionProyecto || 
        getSessionItem("last_descripcionProyecto") || 
        "Sin descripción"
    );
    ...
    // -------------------- GUARDAR EN sessionStorage (Persistencia del Proyecto y Departamento) --------------------
    useEffect(() => {
        ...
        // Se asume que los datos de depId, depNombre, depSlug se guardaron en sessionStorage
        // antes de navegar a esta vista (e.g., en la vista de inicio de departamento).
    ...
    const fetchTareas = useCallback(async () => {
        // ¡CAMBIO CLAVE! El token ahora se lee de sessionStorage.
        ...
        try {
            ...
            // --- 1. Buscar ID del Proyecto si no está en estado/sesión ---
            if (!proyectoId && proyectoSlug) {
                ...
                if (resProyecto.ok) {
                    ...
                    // Actualizar estados y GUARDAR EN sessionStorage si se encontró por slug
                    setIdProyecto(dataProyecto.id_proyecto);
                    setNombreProyecto(dataProyecto.p_nombre); 
            ...
            // --- 2. Obtener tareas con el ID (ahora garantizado) ---
    ...
    }, [idProyecto, proyectoSlug, navigate, depSlug]);
    ...
    return (
        <Layout
            titulo={`TAREAS DEL PROYECTO - ${depNombre.toUpperCase()}`}
            sidebar={
                <MenuDinamico 
            ...
            <div className="tareas-acordeon">
                {tareas.length === 0 ? (
                    ...
                    tareas.map((tarea, index) => {
                        ...
                        const estatusClass = tarea.t_estatus ? tarea.t_estatus.toLowerCase().replace(" ", "-") : 'desconocido';
                            ...
                            <div key={tarea.id_tarea} className={`tarea-item ${estatusClass}`}>
                                <div className="tarea-titulo" onClick={() => toggleExpand(tarea.id_tarea)}>
                                    ...
                                    <div className="tarea-titulo-derecha">
                                        <span className={`tarea-estatus ${estatusClass}`}>{tarea.t_estatus || 'Sin Estatus'}</span>
