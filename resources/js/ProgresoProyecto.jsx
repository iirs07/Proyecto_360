...
export default function ProgresoProyecto({ 
  ...
  const getColorProgreso = (porcentajeEspecifico = null) => {
    ...
    if (porcentaje < 70) return "#ffa500"; // Naranja
  ...
  const getGradientProgreso = () => {
    ...
    if (p < 70) return "linear-gradient(135deg, #ffa500 0%, #ffc04d 30%, #ffa500 70%, #ff9500 100%)";
  ...
  // Configuraciones de tamaño MEJORADAS para gráfica más grande
  ...
  // Componente de Gráfica Circular MEJORADA - CON EFECTOS DE DESTELLO
  const GraficaCircular = () => {
    const config = getTamañoConfig();
    const radio = (config.circular / 2) - (config.strokeWidth / 2);
    ...
    return (
      <div className="prog-proy-contenedor-circular-mejorado">
        ...
        >
          {/* ELEMENTOS DE DESTELLO PARA EL CÍRCULO - AGREGADOS */}
          ...
          >
            ...
            {/* Punto indicador más prominente */}
            {progreso > 0 && progreso < 100 && (
              <circle
                cx={config.circular / 2 + radio * Math.cos((progreso * 3.6 - 90) * (Math.PI / 180))}
                cy={config.circular / 2 + radio * Math.sin((progreso * 3.6 - 90) * (Math.PI / 180))}
                r="6"
          ...
          {/* Texto central MEJORADO - CON EFECTO DE DESTELLO */}
          <div className="prog-proy-texto-circular-mejorado">
            {/* DESTELLO PARA EL TEXTO - AGREGADO */}
            <div className="prog-proy-destello-texto"></div>
            <span 
  ...
  // Componente de Barra de Progreso - CON EFECTOS DE LUZ
  const BarraProgreso = () => {
    const config = getTamañoConfig();
  ...
  const TarjetaEstadistica = ({ tipoTarjeta, valor, etiqueta, Icono }) => {
    ...
    return (
      <div 
        className={`prog-proy-tarjeta-estadistica prog-proy-compacta prog-proy-${tipoTarjeta}`}
        style={{
