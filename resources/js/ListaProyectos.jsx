...
import "../css/ListaProyectosVer.css";
import { 
  FaCalendarAlt, 
  FaTasks, 
  FaSearch,
...
function ListaProyectos() {
  ...
  // Llamada inicial con loader
  useEffect(() => {
    cargarProyectos();
  }, [cargarProyectos]);

  // Auto-refresh cada 5 segundos sin loader
  ...
  return (
    <Layout titulo="PROYECTOS" sidebar={<MenuDinamico activeRoute="ver" />}>
          ...
          <div className="lp-filtros-container">
            <div className="lp-search-filter-wrapper">
              ...
              <div className="lp-search-box">
                ...
                />
                {busqueda && (
                  <button className="lp-search-clear-btn" onClick={() => setBusqueda("")}>
                    <FiX />
                  </button>
                )}
              </div>
            ...
            {busqueda && (
              <div className="lp-search-results-info">
                <span className="lp-results-count">
                  {proyectos.filter((p) => p.p_nombre.toLowerCase().includes(busqueda.toLowerCase())).length}
                </span>{" "}
        ...
        <div className="lp-proyectos-grid">
          {loading ? (
            ...
            proyectosFiltrados.map((p) => {
              ...
              const esFinalizado = p.p_estatus && p.p_estatus.toUpperCase() === "FINALIZADO";

              return (
                <div key={p.id_proyecto} className={`lp-proyecto-card ${getBorderClase(p.p_estatus)}`}>
                  {/* HEADER */}
                  ...
                  <div className="lp-proyecto-actions">
                    {p.total_tareas === 0 ? (
                      <button className="lp-btn-icon" onClick={() => agregarTarea(p.id_proyecto, p.p_nombre)}>
                        <FaPlus />
                        <span>Agregar primera tarea</span>
                      </button>
                    ) : (
                      <button className="lp-btn-icon" onClick={() => verTareas(p.id_proyecto)}>
                        <FaEye />
